defmodule MultiArrayTest do
  ...
  test "an array's values can be read", context do
    arr = context[:list] |> from_list(6,2)

    assert {0,0} |> get_value(arr) == 1
    assert {1,0} |> get_value(arr) == 3
    assert {2,1} |> get_value(arr) == 6
    assert {4,1} |> get_value(arr) == 10

    assert {6,1} |> get_value(arr) == nil
  end

  test "arrays can be converted back into lists", context do
    arr = context[:list] |> from_list(3,4)
    assert arr |> to_list == 1..12 |> Enum.to_list
  end

  test "a values position in an array can be found", context do
    arr = context[:list] |> from_list(2,6)

    assert arr |> find_index(1) == {0, 0}
    assert arr |> find_index(3) == {0, 2}
    assert arr |> find_index(5) == {0, 4}
    assert arr |> find_index(7) == {1, 0}
    assert arr |> find_index(9) == {1, 2}
    assert arr |> find_index(12) == {1, 5}
  end

  test "a non-present value returns nil", context do
    arr = context[:list] |> from_list(2,6)
    assert arr |> find_index(0) == nil
  end

  test "a multi-array has dimension properties", context do
    arr = context[:list] |> from_list(2,6)
    {row, col} = arr |> get_dimensions
    assert row == 2
    assert col == 6
  end
